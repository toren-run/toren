...
- `telegram.bot_token_env` names the env var holding this agent's Telegram bot token — the standard way to put an agent on Telegram. (The all-agents fleet bot via `TELEGRAM_BOT_TOKEN` is an operator tool.) Details in [Telegram](../channels/telegram.md).
