...
- Model routing is by prefix: `mock/` (offline echo), `anthropic/` (needs `ANTHROPIC_API_KEY`), `openai/` (needs `OPENAI_API_KEY`), `bedrock/` (Amazon Bedrock via the Converse API — the model id follows the prefix, e.g. `bedrock/us.anthropic.claude-opus-5-v1:0`; auth is the standard AWS credential chain, no API key exists, region from `AWS_REGION`). Subagents may each use different models. The provider SDKs load lazily: a deployment that never routes a prefix never parses that SDK.
