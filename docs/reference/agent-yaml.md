...
- Model routing is by prefix: `mock/` (offline echo), `anthropic/` (needs `ANTHROPIC_API_KEY`), `openai/` (needs `OPENAI_API_KEY`). Subagents may each use different models. The provider SDKs load lazily — a deployment that never routes a prefix never parses that SDK.
- `mock/echo` is fully offline and deterministic: it replies `echo(<the task's input>)` and never calls tools — useful for asserting orchestration without a model bill. Mock models get no default context window, so they never trigger compaction.
