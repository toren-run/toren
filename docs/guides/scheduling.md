...
**Downtime:** if all workers are down across fire times, the missed occurrences collapse into one catch-up run on revival, and the fire record keeps the originally scheduled time — lateness is visible in the log, never silently swallowed. `next_fire_at` then advances to the next *future* occurrence.
