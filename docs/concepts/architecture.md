...
Locally, Postgres does triple duty — state, queue, event log — so the whole dev stack is one dependency. In AWS the same agent runs unchanged; only the bindings differ.
...
CLI / code ──start──▶ run row + RunCreated ──tick msg──▶ QUEUE
   ...
   guardians (periodic): re-nudge any non-terminal run — heals lost messages
```
